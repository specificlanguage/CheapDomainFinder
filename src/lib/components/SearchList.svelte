<script lang="ts">
    ...
    async function getPrices() {
        ...
        isPremium = premium;
        const res = await fetch(`api/prices?domain=${$domain}&premium=${isPremium}`).then((r) =>
            r.json()
        );
...
{#if !available && available !== undefined}
    <div class="unavailable-box">
        <div class="unavailable-text">
            Oops! <b>{searched_domain}</b> isn't available! Try another domain.
        </div>
    </div>
{:else if available}
    <div class="available-box">
        <div class="available-text">
            ...
            <div style="font-size: 16px">
                Please note that prices may vary on the registrar's page. Prices are listed as rates
                per year.
            </div>
        </div>
    </div>
    {#if isPremium}
        <div class="notice-box">
            <div class="notice-text">
                This is a <b>premium domain</b>; prices are marked up significantly, and not all
                registrar prices are listed here. Consider searching for something else.
            </div>
        </div>
    {/if}
{/if}
